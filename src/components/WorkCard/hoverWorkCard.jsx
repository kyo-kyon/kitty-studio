const WorkCardHover = ({ item, compact = false, index }) =>{
  return (
    <article className={`work-card ${compact ? 'work-card--compact' : ''}`}>
      <div 
        className={`work-image compact work-image--${item.style}`} role="img" aria-label={`${item.title} project preview `}
        onMouseEnter={(e) => {
